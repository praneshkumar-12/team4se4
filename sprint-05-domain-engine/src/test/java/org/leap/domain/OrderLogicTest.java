...
class OrderLogicTest {
    ...
    void rule8_shouldRejectDuplicateIdempotencyKey() {
        ...
        // acceptOrder no longer debits the account, so no top-up is needed
        // here: the duplicate is rejected for its idempotency key alone.
    ...
    void rule8_shouldAllowDifferentIdempotencyKey() {
        ...
        // acceptOrder no longer debits the account, so no top-up is needed
        // here: the account can still afford the second order unchanged.
