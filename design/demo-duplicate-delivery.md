...
This is the runnable script for proving that a duplicate delivery of the
...
**Why this is not a corner case.** Kafka's delivery guarantee for `orders`
is at-least-once, not exactly-once — the alternative would be a broker that
can silently drop a trade, which is worse. That means the Trade Executor
*will* see the same order message redelivered under normal operation
(consumer rebalances, a retry after a slow commit, etc.), not only in this
staged demo. Guard 1 is load-bearing for correctness in production, not a
nicety added for this ticket.
...
### 6. Confirm there is exactly one trade-events message for this order
...
kafka-console-consumer \
  ...
  --topic trade-events \
  --property print.key=true \
  --from-beginning | grep '"orderId":123'
```

Exactly one line should match this order's ID — the original fill/reject
event — never two.
...
Being able to reproduce this once is not the same as the team being able to
give this demonstration live, on demand, at any point in the review.
**Rehearse this as a team before the review** — confirm everyone knows
where `KAFKA_BOOTSTRAP_SERVERS` points in the demo environment, has a
`captured-order.txt` ready (or knows how to capture one live), and knows the
`docker logs` / `psql` commands above by muscle memory.
